...
def extract_ppg_features(ppg_signal, fs=100):
    ...
    if len(peaks) >= 2:
        st_segments = []
        for i in range(min(len(peaks)-1, 10)):  # 限制最多10个周期
...
def main():
    ...
    for i, zf in enumerate(zip_files):
        ...
        if (i+1) % 100 == 0:
            ...
            print(f"已处理 {i+1}/{len(zip_files)} 个文件, 耗时: {elapsed:.1f}秒, 成功: {len(all_features)}")
    ...
    print(f"\n最终样本分布:")
    print(f"正样本: {np.sum(y == 1)}")
    print(f"负样本: {np.sum(y == 0)}")
