...
def main():
    ...
    for i, zf in enumerate(zip_files[:20]):
        ...
        if (i+1) % 100 == 0:
            print(f"已处理 {i+1}/{len(zip_files)} 个文件")
    ...
    print(f"特征已保存到: E:\\data\\dataset\\ecg_features.csv")
    
    # 读取标签文件
    labels_df = pd.read_csv(r'E:\data\dataset\attachments\标签对应表.txt')
    
    # 创建简单标签（基于外部ID前缀）
    # YZ, HR前缀为正样本（有心血管疾病），其他为负样本
    features_df['label'] = features_df['file'].apply(lambda x: 1 if any(prefix in x for prefix in ['YZ', 'HR']) else 0)
